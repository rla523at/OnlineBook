...
## Message Loop
Message Loop(메시지 루프)는 Windows 프로그래밍에서 중요한 역할을 하는 구조로, 애플리케이션이 사용자 입력(키보드, 마우스) 및 시스템 메시지를 처리할 수 있도록 하는 핵심적인 기능입니다. 메시지 루프는 애플리케이션이 실행되는 동안 지속적으로 실행되며, 메시지를 받아들이고 처리하는 역할을 합니다.
