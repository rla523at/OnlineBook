...
세션을 유지하는 대표 방식은 2가지다.

#### 서버 세션 방식(전통적)

* 서버가 세션 정보를 서버 메모리/DB/Redis 등에 저장
* 클라이언트(브라우저)에는 세션 ID만 쿠키로 저장
* 요청마다 세션 ID를 보내면, 서버가 그 ID로 로그인 상태를 조회

예시 흐름

1. 로그인 성공 → 서버는 `session_id=abc123` 같은 값을 생성
2. 서버는 `Set-Cookie: session_id=abc123`로 브라우저에 저장시킴
3. 이후 요청마다 브라우저가 `Cookie: session_id=abc123`를 자동 전송
4. 서버는 `abc123`을 키로 세션 저장소에서 사용자 정보를 조회
