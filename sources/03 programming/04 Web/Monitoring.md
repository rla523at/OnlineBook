...
- Grafana provisioning
  - Grafana가 datasource와 dashboard 같은 설정 대상을 UI에서 직접 입력하지 않고 파일에서 읽어 생성하거나 갱신하는 방식이다.
...
`Grafana provisioning`은 Grafana가 datasource와 dashboard 같은 설정 대상을 파일에서 읽어 생성하거나 갱신하는 방식이다. 여기서 설정 대상은 Grafana 안에서 관리되는 datasource, dashboard, folder 같은 단위다.

datasource provisioning은 Prometheus 같은 datasource의 이름, type, URL, 기본 datasource 여부를 YAML 파일에 둔다. dashboard provisioning은 Grafana가 dashboard 파일을 읽을 provider와 filesystem path를 YAML 파일에 둔다. dashboard 화면의 panel 구성은 별도 dashboard JSON에 둔다.

이 문서에서 `Grafana provisioning`이라고 부를 때는 Grafana의 provisioning 기능으로 datasource와 dashboard를 읽는 동작을 뜻한다. Prometheus의 scrape 설정도 파일로 관리되지만, `Grafana provisioning`과 같은 기능 이름으로 부르지는 않는다.
