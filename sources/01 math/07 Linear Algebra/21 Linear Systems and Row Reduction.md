...
Row operation의 목적은 equation의 leading variable이 계단 모양으로 드러나게 만드는 것이다. Nonzero row에서 가장 왼쪽에 있는 nonzero entry를 그 row의 `leading entry`라고 한다.
...
REF만으로도 back substitution을 통해 해를 구할 수 있다. RREF에서는 각 pivot variable이 free variable에 어떻게 의존하는지가 matrix에 직접 나타난다.
